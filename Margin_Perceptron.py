...
def train(X_train, y_train, epochs=10, lr=0.01, use_bias=True, margin=0):
    hist = History(epochs)
    ...
    for epoch in range(epochs):
        ...
        for x,y in zip(X_train,y_train):
            ...
            if predicted*y < margin :                             #### if mistake
...
# ### Train the classifier: For different cross validsations
...
for margin in [1,0.1,0.01]:    
    ...
    for fold in folds:
        ...
        for lr in [1,0.1,0.01]:
            w, b, hist, updates = train(x_train, y_train, epochs=10, lr=0.01, use_bias=True,margin=margin)
    ...
    for lr_id in [i for i in range(len(results[1]['lr']))]:
        for fold in [1,2,3,4,5]:
            ...
            if avg_acc < acc/len(list(results.keys())) :
                ...
                pass
    print('For margin {} we found lr = {} with Maximum Average Accuracy {} '.format(margin, results[1]['lr'][tmp], avg_acc))
    print('='*100)
    print('='*100)
